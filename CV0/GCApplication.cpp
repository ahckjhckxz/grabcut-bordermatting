...
int GCApplication::nextIter()
{
	...
	if (isInitialized)
	{
		...
		//grabCut(*image, mask, rect, bgdModel, fgdModel, 1, GC_CUT);
	}
	else
	{
		if (rectState != SET)
		...
		if (lblsState == SET || prLblsState == SET)
		{
			...
			//grabCut(*image, mask, rect, bgdModel, fgdModel, 1, GC_WITH_MASK);
		}
		else
		{
			gc.GrabCut(*image, mask, rect, bgdModel, fgdModel, 1, GC_WITH_RECT);
			//grabCut(*image, mask, rect, bgdModel, fgdModel, 1, GC_WITH_RECT);
		}
		isInitialized = true;
	}
	iterCount++;
